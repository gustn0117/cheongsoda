...
export default function QuoteForm() {
  ...
  return (
    <section id="contact" className="relative overflow-hidden py-16 scroll-mt-20 sm:py-32">
      ...
      <div className="container-px mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-center lg:gap-10">
          <div className="order-1 text-white lg:order-1">
            ...
            <div className="mt-6 grid gap-2 sm:mt-10 sm:gap-3">
              {[
                { v: '50팀+', l: '전국 직영팀 서비스 지역' },
                { v: '24시간', l: '카카오톡 상담 응대' },
                { v: '평당 10,000원~', l: '정직한 가격' },
                { v: '1억원', l: '대물 배상책임 보험' },
              ].map((b) => (
                <div
                  key={b.l}
                  className="flex items-center justify-between gap-3 rounded-2xl border border-white/15 bg-white/5 px-4 py-3 backdrop-blur sm:px-5 sm:py-4"
                >
                  <span className="text-[13px] text-white/70 break-keep sm:text-sm">{b.l}</span>
                  <span className="text-sm font-extrabold text-white sm:text-base shrink-0 number-tabular">{b.v}</span>
                </div>
              ))}
            </div>

            <div className="mt-6 grid grid-cols-1 gap-2 sm:mt-10 sm:flex sm:flex-wrap sm:items-center sm:gap-3">
              <a
                href={COMPANY.kakao}
                target="_blank"
                rel="noreferrer"
          ...
          {/* Form card */}
          <div id="quote-card" className="relative order-2 scroll-mt-24 lg:order-2">
            ...
            >
              <h3 className="text-xl font-extrabold text-ink sm:text-2xl">청소 견적 신청</h3>
              <p className="mt-1 text-[13px] text-ink/60 sm:text-sm">
